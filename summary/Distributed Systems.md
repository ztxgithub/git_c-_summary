...
                                        
            (4) 应用
                    在一个集中式集群中, 以 Mesos 为例，Mesos 包括 节点和 slave 节点, slave 节点启动后是主动去和 节
                    点建立连接, 但建立连接的条件是需要知道 节点的 IP 地址和状态等.   节点启动后会将自己的 IP 地址和状态
                    等写入 ZooKeeper 中，这样每个 slave 节点启动后都可以去找 ZooKeeper 获取 的信息。而每个 slave 节点与
...
                    b. 一个 Kubernetes 集群, 主要由 节点和 Worker 节点组成，以及客户端命令行工具 kubectl 和其他附加项
                    c. 由 API Server, Scheduler, Cluster State Store 和 Control Manger Server 进程组成
...
                            kubelet： 通过与 API Server 进行通信, 接收 节点请求或命令(根据调度策略), 在 Worker 节点上
...
                            Borg 的 直接对接用户应用, 也就是说用户可以向 Borg 的 直接请求任务. 但 Mesos 不可以,
                            Mesos 只负责底层资源的管理和分配，并不涉及存储、 任务调度等功能, 因此 Mesos 对接的是 
                            Spark、Hadoop、Marathon 等框架, 用户的任务需要提交到这些框架上. Mesos 的任务调度框架是双层结构
                            
                    b. 组成
                            一个集群包括 Mesos 和多个 Mesos Agent. Mesos 运行在中央服务器, Mesos Agent 运行
                            在节点服务器上. 
                            
                            Mesos 负责收集和管理所有 Agent 所在服务器的资源和状态, 并且对接 Spark、Hadoop 
                            等框架, 将集群中服务器的资源信息告知给这些框架, 以便这些框架进行任务资源匹配和调度.
                            Mesos 通常采用一主两备的方式, 以方便故障处理和恢复。而 Mesos 的选主策略是 ZAB 算法
                            
...
                                    通过容器分配任务则能更好地进行“装箱”
                                                        
                            2. 可扩展性, Mesos 可扩展设计的关键是两级调度架构, 其中 Framework 进行任务调度, Mesos  
                                        进行资源分配. 由于 不必知道每种类型的应用程序背后复杂的调度逻辑, 不必为每个任务做调度
                                        可以用非常轻量级的代码实现，更易于扩展集群规模
                                        
                            3. 模块化. 每接入一种新的框架, 无需增加新的代码, 并且 Agent 模块可以复用
                            
                    d. Mesos 支持容器部署
                            Mesos 本身只负责资源管理, 不负责任务调度. 但 Mesos 可以对接不同的框架, Mesos + Marathon 可以支持
                            容器调度和部署. Marathon 支持容器的调度(Docker 和 cgroups), 将容器部署请求发给 Mesos ,
                            Mesos 再将请求转发给 Mesos Agent, Mesos Agent 的执行器会将容器拉起
                            
    3. 与 Slave 保活
            通过 TCP 长连接和心跳机制协同进行.
```
...
                    a. 特性
                            支持数据的持久化, 支持多种数据结构, 支持数据的备份(/Slave 模式的数据备份)
                           
                    b. 简介
...
                      
    3. 保活机制
            (1) 和集中式架构不同, 非集中式没有 和 Slave 之说, 如果每个节点间建立 TCP 长连接浪费资源
...
       多个集群的 向这个控制中心进行注册, 控制中心会管理所有注册集群的状态和资源信息, 控制中心接收到任务后会根据任务和集群信息
...
                    a. 资源管理集群是由一个 节点和多个 Slave 节点组成的集中式系统.每个集群有且仅有一个 节点, 
                       负责管理 Slave 节点, 并对接上层框架；Slave 节点向 节点周期汇报资源状态信息,
...
                       执行器（Executor）组成，调度器可以从 节点获取集群节点的信息 ，执行器在 Slave 节点上执行任务
                       
            (2) 过程
                    第一步: 框架向 Mesos 注册
                    第二步: Mesos Slave 节点定期或周期向 Mesos 上报本节点的空闲资源
                    第三步: Mesos 的 Scheduler 进程收集所有节点的空闲资源信息, 并以 Resource Offer 的方式将空闲资源发送给
                           注册的框架
                    第四步: 框架的 Scheduler 接收到 Mesos 发送的资源后, 进行任务调度与匹配, 匹配成功后, 
                           将匹配结果下发给 Mesos ，并由 Mesos 转发给相应节点的执行器执行任务(执行器在 slave 中)
                           
            (3) 第一层调度算法(资源分配算法)
...
                           2 个
                    第三步, 被分配 Map 作业的 Worker, 开始读取子任务的输入数据, 并从输入数据中抽取出 键值对,
...
            
    3. 应用 Apache Storm
            (1) Storm 集群上有两种节点, 即主节点（ Node）和工作节点（Worker Nodes）
...
                    为了防止 Nimbus 单点故障, 引入 ZooKeeper 集群来增加可靠性,   Node 与 Worker Node 之间的交互通过
...
                                       都进行了主从设计, 即每个 Broker 分为 Broker 和 Broker Slave. 可读可写, 
                                       Slave 只读. 每次 Broker 会把接收到的消息同步给 Broker Slave，以实现数据备份.
                                       
            (2) 过程
