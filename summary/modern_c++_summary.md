...
6. 实现 class 类禁止拷贝和赋值
        template <typename T>
            ...
            {
                    …
                smart_ptr(const smart_ptr&) = delete;  // 禁止拷贝
                
                smart_ptr& operator=(const smart_ptr&) = delete;  // 禁止赋值
                    …
            };
    ...
    　　this 对象完全不受任何影响. 无论拷贝构造成功与否，结果只有赋值成功和赋值没有效果两种状态，而不会发生因为
      ...
      　实例:
            ...
            smart_ptr<shape> ptr2{ptr1}; // 编译出错, 没有拷贝构造函数
            smart_ptr<shape> ptr3;
            ptr3 = ptr1; // 编译出错, 这就是 unique_ptr 的优点,编译时就报错, 而 auto_ptr 则不会报错
            /*
                这里类内重写了 operator=(赋值操作符), 传入的参数 smart_ptr rhs 是需要通过 移动构造函数而不是拷贝构造函数
                std::move(ptr1) 作用是把一个左值引用强制转换成一个右值引用(T&& )，而并不改变其内容
            */
    ...
    8. 
        ...
        public:
            ...
            /*
             *  拷贝构造函数
             *  这里的 template <typename U> 主要是为了 smart_ptr<circle> 隐式的转化为　smart_ptr<shape>
             * */
            template <typename U>
...
```shell
    ...
    6. 移动语义
            (1) c++11 之后，使用容器类时, 通过移动语义可以减少临时对象的产生,  优化其性能.
                ...
                1. 调用构造函数 string(const char*), 生成临时对象1, "hello " 复制 1 次
                
                2. 调用 operator+(string&&, const string&), 直接在临时对象1 上面执行追加
                   操作, 并把结果移动到临时对象 2；name 复制 1 次
                   
                3. 调用 operator+(string&&, const char*), 直接在临时对象 2 上面执行追加操
...
```shell
    ...
    7. 成员函数说明符
            ...
            (2) delete 成员函数
                    class shape_wrapper {
                        shape_wrapper(const shape_wrapper&) = delete;  // 不允许外部使赋值操作符
                        shape_wrapper& operator=(const shape_wrapper&) = delete;// 不允许外部使赋值操作符
                    };
    ...
    3. optional 
            ...
            (3) 具体的用法
                   ...
                   c. 如果 T 类型支持拷贝构造或者移动构造，在构造 optional<T> 时也可以传递一个 T 的左值或右值来将 T 对象拷贝或
                      移动到 optional 中
...
```shell
    ...
    3. atomic 类型
            ...
            (2) atomic
                    a. std::atomic 对 int, char, bool等数据结构进行原子性封装，在多线程环境中，对 std::atomic 对象的访问不会
                       造成竞争-冒险。利用 std::atomic 可实现数据结构的无锁设计
                    b. 成员函数
                            1. 
                                atomic (const atomic&) = delete;　// 拷贝构造函数被禁止
                                atomic& operator= (const atomic&) = delete;  // atomic 对象间赋值被禁止
...
```shell
    ...
    4. Boost.Conversion (数据类型的转化, int -> string, string -> int), 不需要任何动态链接库
            ...
            auto sTof = lexical_cast<string>(srcf);  // sTof 为"11111.2"
    ...
    5. Boost.Program_options(运行的选项处理库) 需要链接 libboost_program_options.so   动态链接库
            ...
            (2) 实例代码
                        ...
                        po::options_description desc("Usage: breaktext [OPTION]...  <Input File> [Output File] \n\n"
                                                             "Available options");
                        desc.add_options()
                                ("locale,L", po::value<string>(&localion), "Locale of the console (system locale by default)")
                                ("lang,l", po::value<string>(&lang), "Language of input (asssume no language by default)")
                                ("width,w", po::value<int>(&width), "Width of output text (72 by default)")
    ...
    2. Catch2: 只需要单个头文件即可使用，不需要安装和链接
            ...
            (3) 例子
                    SCENARIO("Int container can be accessed and modified [container]")
                    {
                        ...
                        {
                            /*
                                GIVEN 之后, 第一个 When 之前为初始化语句, 每次执行 When 都要调用初始化语句
                             */
...
```shell
    1. std::back_inserter() 函数,在尾部插入元素.
            ...
            功能: 构造一个迭代器在容器末尾添加元素
            返回值: 返回的是新增的迭代器
            注意: 使用 back_inserter 的容器是有 push_back 成员函数的容器, 比如 vector, deque and list
